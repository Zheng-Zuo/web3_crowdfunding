...
import { parseErrorMessages, updateCampaign } from '@/utils/actions';
...
export default function EditModal({ id, title, description, imageURL, cost, expiresAt }: Props) {

  const editTx = updateCampaign()
  const { address } = useAccount()
  const { chain } = useNetwork()

  const { openEdit, setOpenEdit, errorConnect, setErrorConnect } = useGlobalStates();
  ...
  useEffect(() => {
    ...
    setValue('expireDate', dayjs(expiresAt));
  }, [title, description, cost, imageURL, expiresAt, setValue]);
  ...
  const onSubmit = async (inputData: inputProps) => {
    ...
    if (address && chain && chain.id === 11155111) {
      ...
      if (!imageFile) {
        ...
        firebaseImageUrl = await uploadImage(imageFile)
      }
      setUploading(false)
      console.log(firebaseImageUrl)

      await editTx.writeAsync({
        args: [
          parseInt(id),
          inputData.title,
          inputData.description,
      ...
      >
        ...
        <Box sx={style} ref={modalContentRef}>
          <form onSubmit={handleSubmit(onSubmit)} className='h-[700px] max-sm:h-[500px] text-gray-900'>
            ...
            {(alertMessage && (errorConnect || editTx.isSuccess || editTx.isError)) && (
              <Alert severity={
                errorConnect || editTx.isError ? 'error' : 'success'
              } sx={{ mt: 2 }}>
                {alertMessage}
              </Alert>
            )}
